#!/usr/bin/env python3
"""
Tempus Matching System wrapper for FastAPI tabbed interface
Based on 04_04_tempus_match_email.py
"""
...
class TempusMatchingSystem:
    """Tempus gene matching system for precision oncology."""
    ...
    async def initialize(self):
        ...
        try:
            ...
            print("Loading Tempus vector database from: tempus_json_index")
            tempus_embeddings = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")
            self.tempus_db = FAISS.load_local("tempus_json_index", tempus_embeddings, allow_dangerous_deserialization=True)
            ...
            tempus_retriever = self.tempus_db.as_retriever(search_kwargs={"k": 4})
            ...
            print(f"❌ Error initializing Tempus system: {str(e)}")
            self._ready = False
            raise
    ...
    def _create_comprehensive_retriever(self):
        ...
        class ComprehensiveRetriever(Runnable):
            def __init__(self, octsu_db, tempus_db, k=8):
            ...
            def get_relevant_documents(self, query, run_manager=None):
                ...
                try:
                    ...
                    # Search Tempus database (genomic reports)
                    tempus_docs = self.tempus_db.similarity_search(query, k=self.k//2)
                    
                    # Combine and return
                    all_docs = octsu_docs + tempus_docs
        ...
        return ComprehensiveRetriever(self.octsu_db, self.tempus_db)
    
    def _create_rag_chain(self):
        """Create the RAG chain for Tempus matching."""
        ...
        You help match Tempus genomic test results to appropriate clinical trials based on specific genetic mutations and biomarkers.

        Your expertise includes:
        - Tempus genomic testing panel and interpretation
        - Clinical trial eligibility criteria and matching
        - KRAS, MSI-H (Microsatellite Instability-High), and related mutations
        ...
        1. **Specific Matching**: When asked to match Tempus reports to studies, provide specific study identifiers and detailed eligibility criteria
        2. **Mutation Focus**: Highlight relevant mutations found in Tempus reports that match clinical trial requirements
        ...
        Focus on Tempus-specific genomic data and ensure all recommendations are based on the provided knowledge base."""
    ...
    async def process_query(self, query: str, chat_history: List[Dict]) -> str:
        """Process a query using the Tempus matching system."""
        try:
            if not self._ready:
                return "Tempus system is not ready. Please try again later."
            ...
            print(f"❌ Error processing Tempus query: {str(e)}")
            return f"Sorry, I encountered an error processing your request: {str(e)}"
    
    def is_ready(self) -> bool:
        """Check if the Tempus system is ready."""
        return self._ready
